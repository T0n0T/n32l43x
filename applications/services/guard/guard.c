...
int task_guard_register(guard_type_t type, guard_handle_func handle, void* data)
{
    ...
    // 如果找到了空闲槽位，则初始化任务
    if (task_id > 0) {
        guard_ins.task[task_id].mask   = GUARD_TASK_MASKED;
...
void task_guard_mask(int task_id)
{
    if (task_id < GUARD_TASK_NUM) {
        guard_atomic_set_mask(&guard_ins.task[task_id].mask, GUARD_TASK_MASKED);
    }
}
...
{
    if (task_id < GUARD_TASK_NUM) {
        guard_atomic_clear_mask(&guard_ins.task[task_id].mask, GUARD_TASK_MASKED);
    }
}
...
{
    for (int i = 0; i < GUARD_TASK_NUM; i++) {
        if (!(guard_ins.task[i].mask & GUARD_TASK_MASKED) && guard_ins.task[i].handle) {
