...
static LONG          lMasterTimeout = MB_MASTER_TIMEOUT_MS_RESPOND; // 存储超时值
...
eMBMasterReqErrCode eMBMasterWaitRequestFinish(void)
{
    ...
    BOOL                xEventHappened   = FALSE;
    ULONG               ulTimeoutCounter = 0;

    // while (1) {
    //     // Process modbus master events to prevent deadlock
    //     eMBMasterPoll();
    //     // Check if the expected events are in queue
    //     if (xEventInQueue) {
    //         if ((eQueuedEvent & EV_MASTER_PROCESS_SUCESS) || (eQueuedEvent & EV_MASTER_ERROR_RESPOND_TIMEOUT) || (eQueuedEvent & EV_MASTER_ERROR_RECEIVE_DATA) || (eQueuedEvent & EV_MASTER_ERROR_EXECUTE_FUNCTION)) {
    //             // Get the event
